...
For proper operation this .py file should be located within the "TOFHunter build" folder with the "Reference Sheets" and "assets" folders as well
...
def run_TOFHunter(filename, export_path, pca_components, unique_spectra, peak_height, prominence, distance):
    ...
    def IFF(spectra, num_randvectors=10000):
        ...
        Parameters:
        - spectra: The data set you want to explore with spectraectra along the rows.
        - num_randvect: Number of random vectors to be generated (should be >= 2000).

        Returns:
        - ind: Indexes of selected spectraectra (sorted by decreasing order of selection frequency).
        - freq: Selection frequency of each spectraectrum in ind.
        """
    ...
    # This next little loop searches through the NISTAMU list to concat matched names within an acceptence to each peak
